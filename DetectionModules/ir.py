...
class IR(DetectionMethod):
    """
    Objects of this class contain the specifications for an IR detection program, as well as a
    method for detecting leaks
    """
    ...
    def __init__(self, simulation_time, gas_field, **kwargs):
        ...
        # clustering of pixels...for example, a comp_res_factor of 4 means that signals are
        # calculated at 1/4 of the pixels on the x and z axes, and 1/16 of all pixels
        ...
        self.find_cost = np.zeros(simulation_time.n_timesteps)
    ...
    def set_attrs(self, time, gas_field):
        ...
        self.time_factor = self.survey_time * gas_field.site_count / (self.survey_interval
                                                                      * self.work_time)
        ...
        # Maintenance costs are estimated as 10% of capital costs distributed evenly throughout each
        # year
        ...
        indexes = np.where(
            np.linspace(0, time.end_time, time.n_timesteps) % self.survey_interval < time.delta_t)
        self.find_cost[indexes] = self.survey_cost

    def detection(self, time, gas_field, atm):
        """
        detection assumes that all leaks are viewed from the side from the same camera position
        relative to the leak source. Reflection off the ground is neglected and plume rise is
        assumed to be small compared to the height of the camera.
        Inputs:
        ...
        if not self.TEMP_FLAG and self.leaks.n_leaks < 0:
            print("IR WARN: why are there less than zero leaks")
            self.TEMP_FLAG = True
        t_index = time.time_index
        if time.current_time % self.survey_interval < time.delta_t:
            log.debug("Day {:.1f}, performing survey. Largest existing: {}".format(
                time.current_time,
                np.max(self.leaks.flux)))

            # Define
            # new leak object based on
            # self.leaks with each leak at a standard height
            #  and
            # sorted by flux
            n_l = self.leaks.n_leaks
            sort_perm = np.argsort(self.leaks.flux)
            flux = np.sort(self.leaks.flux)
            # calculate absorption values for the plume. If e_a, e_g, t_g t_plume and t_a are the
            # same on consecutive timesteps, methane_pnnl returns the same results without repeating
            # any calculations
            e_g = atm.e_g[time.time_index]
            e_a = atm.e_a[time.time_index]
            t_g = atm.ground_temp[time.time_index]
            t_a = atm.a_temp[time.time_index]
            t_plume = gas_field.t_plume
            ...
            counter = 0
            detection_level = self.pixel_detection(atm, flux[-1], k_av, nep, tec, t_index, e_a)
            if detection_level < self.detection_limit:
                detect = []
            else:
                while high - low > 1:
                    counter += 1
                    leak_ind = low + (high - low) // 2
                    total_pixels = self.pixel_detection(atm, flux[leak_ind], k_av,
                                                        nep, tec, t_index, e_a)
                ...
                # print("{} largest leaks missed: {}".format(self.name, flux[(low - 3):high]))
                # print("{} smallest leaks found: {}".format(self.name, flux[high:(high + 3)]))
            ...
            if len(detect) > 0:
                smallest_found = np.min(self.leaks.flux[detect])
            # Delete found leaks
    ...
    def pixel_detection(self, atm, flux, k_av, nep, tec, t_index, e_a):
        ...
        pixels = 0
        pixels0 = 0
        # Store the camera orientation as a bool (This structure allows for other orientations to be
        ...
        for j in range(0, self.xpoints):
            ...
            if isy:
                # Calculate sigma_z
                sigma = atm.l[t_index] * self.x[j] / \
                        (1 + self.x[j] / atm.a[t_index]) ** atm.q[t_index]
            else:
                # calculate sigma_y
                sigma = atm.k[t_index] * self.x[j] / \
                        (1 + self.x[j] / atm.a[t_index]) ** atm.p[t_index]
    ...
    def methane_pnnl(self, e_g, e_a, t_g, t_plume, t_a):
        """
        Calculates the light absorption properties of a methane plume using the Pacific Northwest
        National Lab library
        Inputs:
        ...
        Return:
            kav     average k value over the range of wavelengths from 3.2 to 3.4 microns for
                    methane
            nep     noise equivalent power (used to determine the minimum detectable change in
                    power) [W]
    ...
    def bb_rad_params(self, t_g, t_plume, t_a, e_g, e_a):
        ...
        # w1g and w2g are the frequency limits over which the camera is sensitive (multiplied by H/K
        # t_g to make them non-dimensional)
        w1g = H * c / (self.lambda2 * K * t_g)
        w2g = H * c / (self.lambda1 * K * t_g)
        # The product n1 * y1 is the integral of the derivative of blackbody radiation with respect
        # to temperature over the range of frequencies that the camera is sensitive to multiplied by
        # a constant.
        n1 = 2 * np.pi * K ** 4 * t_g ** 3 / (H ** 3 * c ** 2)
        temp_y1 = -np.exp(-w1g) * (720 + 720 * w1g + 360 * w1g ** 2 + 120 * w1g ** 3
                                   + 30 * w1g ** 4 + 6 * w1g ** 5 + w1g ** 6)
        temp_y2 = -np.exp(-w2g) * (720 + 720 * w2g + 360 * w2g ** 2 + 120 * w2g ** 3
                                   + 30 * w2g ** 4 + 6 * w2g ** 5 + w2g ** 6)
        y1 = temp_y2 - temp_y1
        y = y1 * n1
        # Noise equivalent power
        nep = y * self.netd * self.a_d / (4 * self.f_number ** 2)

        # Calculate the power incident on a pixel from a blackbody at the ground temp, plume temp
        # and atmosphere temp.
        ppixelg = self.pixel_power(t_g)
        ppixelp = self.pixel_power(t_plume)
        ppixela = self.pixel_power(t_a)
        # tec is the signal in a pixel caused by an infinitely thick plume in an atmosphere with an
        # emissivity of zero.
        # tec is a useful value because the signal due to a finite plume in a real atmosphere is
        # proportional to tec.
    ...
    def pixel_power(self, temp):
        """
        Calculate the the power incident on a pixel from an infinite blackbody emitter at a given
        temperature.
        Inputs:
...
class MIR(IR):
    """
    Class for a manually operated infrared detection method. The plume is assumed to be observed
    from the side in the MIR class and from above in the AIR class. Otherwise, the two classes are
    nearly identical with different default values.
    """

    def __init__(self, simulation_time, gas_field, **kwargs):
        IR.__init__(self, simulation_time, gas_field)
        self.name = "MIR"
        # -------------- Process details --------------
        ...
        self.distance = 10  # meters from leak
        ...
        # Camera orientation relative to the wind (sign is irrelevant)
        ...
        self.set_attrs(simulation_time, gas_field)

    def __str__(self):
        msg = "MIR: Survey interval (days): {}  Obs. distance (m): {}".format(self.survey_interval,
                                                                              self.distance)
        return msg
...
class AIR(IR):
    """
    Class for a manually operated infrared detection method. The plume is assumed to be observed
    from above in the AIR class and from the side in the MIR class . Otherwise,the two classes are
    nearly identical with different default values.
    """

    def __init__(self, simulation_time, gas_field, **kwargs):
        IR.__init__(self, simulation_time, gas_field, **kwargs)
        self.name = "AIR"
        # -------------- Process details --------------
        self.survey_interval = 14  # days
        self.survey_speed = 5  # m/s
        self.drive_speed = 15  # m/s
        # allocates 30% of flight time to landing, refueling, and launching quadcopter
        self.setup_factor = 1.3
        self.work_time = 5  # hours/day on average ~= 35 hours per week.
        # -------------- Detection technology properties --------------
        self.distance = 20  # meters
        self.detection_limit = 2000  # pixels
        # capital_0 is spread over the maximum possible number of wells
        # dollars (covers 90k for camera, 30k for truck, 50K for UAV, 10k for optics and 2500
        self.capital_0 = 193000
        # for computers)
        self.labor = 100  # dollars/hour
        self.lifetime = 10 * 365  # days
