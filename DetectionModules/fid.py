"""
FID simulates a Flame Ionization Detector LDAR program. The module defines a new subclass of
DetectionMethod. The class has two methods: an initialization method and a detection method.
"""
...
class FID(DetectionMethod):
    """
    Objects of this class contain the specifications for an FID detection program, as well as a
    ...
    def __init__(self, simulation_time, gas_field, **kwargs):
        ...
        self.work_time = 5  # hours/day on average ~= to 35 hours per week.
        ...
        self.capital = np.zeros(simulation_time.n_timesteps)
        helper_functions.replacement_cap(simulation_time, self)

        # maintenance costs are estimated as 10% of capital per year
        self.maintenance = [self.maintenance_0 * simulation_time.delta_t / 365, ] * simulation_time.n_timesteps  # $
        ...
        for ind in range(0, simulation_time.n_timesteps):
            curr_time = ind * simulation_time.delta_t
            if curr_time % self.survey_interval < simulation_time.delta_t:
