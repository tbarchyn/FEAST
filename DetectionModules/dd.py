...
class DD(DetectionMethod):
    def __init__(self, simulation_time, gas_field, **kwargs):
        ...
        self.location_time = (gas_field.components_per_site / self.survey_speed / self.n_sniff
        ...
        self.maintenance = [self.maintenance0 * simulation_time.delta_t / 365] * simulation_time.n_timesteps  # $/year
    ...
    def detection(self, time, gas_field, atm):
        ...
        if time.current_time % self.repair_interval < time.delta_t and time.current_time != 0:
            ...
            self.repair_cost[time.time_index] += \
                sum(sample_wr(gas_field.repair_cost_dist.repair_costs, int(count_found)))
        ...
        phi = simulation_functions.gauss_leak_model(self.x, self.y, self.z, self.leaks, atm,
                                                    time.time_index, index=indexes)
