...
def results_analysis(directory):
    ...
    Return:
        ...
        leakage_timeseries  Array of leakage in each LDAR program in each realization at each time
                            step
    """
    ...
    return no_repair_npv, null_npv, leaks_found, leakage_timeseries
...
def npv_calculator(filepath):
    """
        ...
        Return:
            no_repair_npv     NPV of each LDAR program compared to a scenario with no repair
                              [k$/well]
            null_npv          NPV of each LDAR program compared to a scenario with only the Null
                              LDAR program [k$/well]
    """
    ...
    null_leakage = np.array(sample.tech_dict["Null"].leakage)
    ...
    for index in range(0, len(techs)):
        ...
        gas_value_no_repair[index] = sum((np.array(sample.no_repair_leakage)
                                          - sample.tech_dict[techs[index]].leakage) /
                                         discount_array) * sample.time.delta_t * 24 * 3600 \
            * sample.econ_settings.gas_price
        repair_cost_no_repair[index, :] = sample.tech_dict[techs[index]].repair_cost / \
            discount_array
        if techs[index] != "Null":
            ind = index - null_correct
            repair_cost_null[ind, :] = repair_cost_no_repair[index, :] - null_repair_cost
            gas_value_n[ind] = sum((null_leakage - sample.tech_dict[techs[index]].leakage)
                                   / discount_array) * sample.time.delta_t * 24 * 3600 * \
                sample.econ_settings.gas_price
            ...
            maintenance_cost_null[ind, :] = sample.tech_dict[techs[index]].maintenance / \
                discount_array
        else:
            null_correct += 1
    ...
    null_npv = {"Capital": cap_n, "Maintenance": main_n, "Repair": n_repair,
                "Finding": find_n, "Gas": gas_value_n}
