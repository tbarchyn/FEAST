...
# purple doesn't show up on black [83, 40, 79]
color_set = [[140, 21, 21], [0, 152, 219], [0, 155, 118], [178, 111, 22],
             [0, 0, 0], [77, 79, 83], [255, 255, 255]]
...
def time_series(results_file, display=True, save=False, file_out=None, interactive=True):
    ...
    print("Timesteps: {}".format(results.time.n_timesteps))
    for key in tech_dict.keys():
        print("{}: numdays: {}".format(key, len(tech_dict[key].leakage)))
    fig = plt.figure()
    ax = fig.add_subplot(111)
    counter = -1
    for tech in tech_dict.keys():
        counter += 1
        tech_dict[tech].line = ax.plot(np.array(range(0, results.time.n_timesteps)) *
                                       results.time.delta_t, np.array(tech_dict[tech].leakage)
                                       / results.gas_field.site_count, label=tech,
...
def summary_plotter(directory, display=True, save=False, file_out=None, interactive=True):
    """
    The NPV for each realization stored in 'directory' is calculated and displayed in a stacked bar
    chart. Each component of the NPV is displayed separately in the chart.
    Inputs:
    ...
    for key in key_list:
        if key == "Total" or key == "Gas":
            ...
            for k in ind:
                ...
                if npv[key][m] >= 0:
                    if m == 0:
                        plt.bar(m, -npv[key][m], width, bottom=neg_bottoms[m],
                                color=color_set[counter], label=key)
                    else:
                        plt.bar(m, -npv[key][m], width, bottom=neg_bottoms[m],
                                color=color_set[counter])
                    ...
                    if m == 0:
                        plt.bar(m, -npv[key][m], width, bottom=pos_bottoms[m],
                                color=color_set[counter], label=key)
                    else:
                        plt.bar(m, -npv[key][m], width, bottom=pos_bottoms[m],
                                color=color_set[counter])
    ...
    plt.errorbar(ind + width / 2, totals, yerr=npv_std, linewidth=5,
                 fmt="o", ms=1, label="Total NPV")
...
def hist_plotter(directory, display=True, save=False, file_out=None,
                 interactive=True, inline=False, bins=None):
    """
    ...
    for tech in techs:
        ...
        ax2 = ax1.twinx()
        counts_temp, bins_temp, patches = ax1.hist(tech_leaks_found[tech], bins,
                                                   normed=0, alpha=0.75)
        ax1.cla()
        counts_temp /= well_count
        ax1.bar(bins_temp[0:n_bins], counts_temp, width=delta,
                color=color_set[ind + 1], label="Leaks found")
